<script>
    ...
    const productEdit = () => {
        let datatable, timeoutSearch, editForm, myDropzone, addButton, modal, formProductOption,
            productAttributeForm, listAttributeStatus, productAttributeReapeater, removeImages;
        ...
        const initDatatable = () => {
            ...
            const initEditAction = () => {
                $(document).on('click', '.edit-button', function() {
                    ...
                    formProductOption.find('input[name="current_price"]').val(productOption
                        .current_price);
                    ...
                    for (let index = 0; index < productOption.product_media.length; index++) {
                        ...
                        mockFile.previewElement.querySelector(".dz-details ").style.display =
                        "none";
                        mockFile.url = url;
                    }
                    ...
                    listAttributeStatus = listAttribute.map(attribute => ({
                        ...
                        values: attribute.values.map(value => {
                            return {
                                id: value.id,
                                text: value.value,
                                isSelected: productOption.attribute_values.find(
                    ...
                    for (let index = 0; index < productOption.attribute_values.length; index++) {
                        ...
                        repeaterItem.find('select[name="product_variable"]').val(attributeValue
                            .attribute_id).trigger('change');
                        repeaterItem.find('select.product-variable-value').val(attributeValue.pivot
                            .attribute_value_id).trigger('change');
                    }
                    // initReapeater();
                    modal.modal('show');
                })
        ...
        formProductOption.submit(function(e) {
            ...
            $.ajax({
                url: id ? ("{{ route('product_option.update') }}") : (
                    "{{ route('product_option.store') }}"),
                method: "POST",
                data: formData,
                contentType: false,
                cache: false,
