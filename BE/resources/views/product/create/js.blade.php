<script>
    $(document).ready(function() {
        ...
        const handleProductOptionRowAdd = (row_collapse) => {
            ...
            productAttributeForm.find('div[data-repeater-list]').attr('data-repeater-list',
                'product_attribute_values_' + row_collapse.index());
            ...
            const handleRowAdd = (row) => {
                row.on('change', 'select.product-variable', function(e) {
                    ...
                    let attributeValues = listAttributeStatus.find((attribute) => attribute
                        .id ==
                    ...
                    variableValue.on('change', function(e) {
                        listAttributeStatus = listAttributeStatus.map(attribute => {
                            if (attribute.id == attributeSelectId) {
                                attribute.values = attribute.values.map(
                                    value => {
        ...
        form.on('submit', function(e) {
            ...
            try {
                ...
                productOptionElements.forEach((element, index) => {
                    let productAttributeElements = element.querySelector(
                        '.product-attribute-form')
                    let product_attribute_values = [];
                    const variableValues = productAttributeElements.querySelectorAll(
                        'select.product-variable-value')
                    ...
                    product_option.push({
                        ...
                        current_price: element.querySelector(
                            'input.option-current-price').value,
                    ...
                    if (image_files.length > 0) {
                        for (let i = 0; i < image_files.length; i++) {
                            dataSend.append('product_option_image_' + index + '[]', image_files[
                                i]);
                        }
                    }
